...
start_timestamp = '1722480600000'  # 开始时间 
end_timestamp = '1722491400000'    # 结束时间
# 将时间戳转换为秒（时间戳单位是毫秒）
...
def load_and_process_data():    
    ...
    if 'timestamp' in data.columns:
        ...
        # 调整时间窗口为每分钟
        data['time_window'] = data['timestamp'].dt.floor('1s')  # 将时间向下取整到最近的1秒
...
def train_model(train_dataloader, val_dataloader):
    ...
    # 当使用 PyTorch Lightning 的 Trainer 类时，某些属性如 loss 和 logging_metrics 已经自动保存，不需要在超参数中再次保存。
    ...
    # 获取并可视化特征重要性
    feature_importances = tft.calculate_feature_importance(val_dataloader)
